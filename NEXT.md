...
- **여기까지 됨** — 명령 두 개. 그중 링크 에러 해독은 MSVC 와 GNU 를 둘 다 읽는다.
  ...
  - **GNU 링크 에러 해독** (0.3.0). `ld`·`lld` 의 `undefined reference to` 를 읽음. 이 링커들은
    이름을 디맹글해서 찍는데 아카이브 인덱스는 맹글된 채라 그냥은 하나도 안 맞음 — `mangle.js` 가
    맹글된 이름의 앞부분을 되살려 접두사로 맞추고, 템플릿처럼 앞이 안 맞는 것은 컴포넌트 순서로
    찾음. libstdc++.a 의 맹글 심볼 7,204개에 돌려 접두사 37.8% · 컴포넌트 46.8% · 못 찾음 11.4% ·
    "모름"으로 답함 4.1%(연산자). 아카이브는 PATH 의 GNU 툴체인 아래에서도 찾음
...
- **다음 할 것** — 메모리 릭 30분 스파이크 하기. `메모리 릭` 절을 볼 것. 그 결과로 릭 기능을 만들지 접을지가 갈린다.
  그것과 별개로 제일 싼 후보는 표준 스트림 축약 표(`Si`·`So`·`Ss`·`Sd`·`Sa`·`Sb`) 다섯 줄을
  `mangle.js` 에 넣는 것 — 지금 못 찾는 11.4% 가 거의 전부 그것이고, 맥·리눅스에서도 잴 수 있다.
- **막힌 것** — 릭 기능은 **만들 값어치가 아직 확인되지 않았다**. `메모리 릭` 절을 먼저 읽을 것.
  나머지는 없음. 아래 "아는 한계" 참고.

## 2026-09-20 에 리눅스 컨테이너에서 확정한 것

**전부 리눅스 클라우드 컨테이너에서 잰 것이다** (gcc 13.3 · clang 18 · binutils 2.42 ·
크로스 `x86_64-w64-mingw32-g++` 13.2). 윈도우 PC 에서 다시 잰 것이 아니므로 아래 셋만 확정이고,
윈도우 실물에서 갈릴 수 있는 것은 그 아래 "아직 안 잰 것" 에 적었다.

- **MinGW `ld` 의 메시지 형식이 리눅스 `ld` 와 같다.** `x86_64-w64-mingw32-ld` 가 실제로 찍은 것을
  받아 봤다 — 앞의 파일 경로 부분만 `win.o:win.cpp:(.text+0x12)` 로 조금 다르고,
  ``undefined reference to `name'`` 부분은 글자 그대로 같다. 전에 "같을 것으로 보나 안 재 봤음"
  으로 적어 둔 항목이 이제 실측이다
- **MinGW 라이브러리 레이아웃이 `gnuLibDirs()` 의 가정과 맞다.**
  `<root>/lib/gcc/x86_64-w64-mingw32/13-posix` 와 `<root>/x86_64-w64-mingw32/lib` 둘 다 실재하고,
  뒤쪽에 Win32 import 라이브러리가 **892개** 있다. `__imp_CreateFileW` 를 붙여넣으면
  `libkernel32.a` 가 나오는 것까지 end-to-end 로 확인했다 (1,022개 아카이브를 148ms 에 훑음)
- **`std` 는 맹글된 이름에서 예외 없이 `St` 로 축약된다.** libstdc++.a 의 7,357개 중 `_ZN3std` 로
  시작하는 것이 **0개**다. 이것을 모르고 만든 `_ZN3std...` 접두사는 영영 안 맞는 것이었다

### 아직 안 잰 것

- **윈도우 PC 의 진짜 MinGW(WinLibs·MSYS2)에서 `gnuLibDirs()` 가 도는지.** 위는 데비안 크로스
  패키지라 root 가 `/usr` 다. 레이아웃 세 자리는 같지만 PATH 에 `gcc.exe` 가 있는 자리에서
  한 단계 올라가 맞는지는 윈도우에서 한 번 돌려 봐야 확정이다
- **MSVC 와 GNU 를 같이 뒤질 때의 실제 소요 시간.** 여기서는 MSVC 아카이브가 없어 GNU 쪽만 쟀다.
  윈도우에서는 1,524 + MinGW 쪽이 더해지므로 한 번 재 보고 1초를 넘으면 그때 본다
...
- **표준 스트림 타입의 축약은 안 다룬다.** `std::basic_istream<char, std::char_traits<char> >`
  는 맹글된 이름에서 `Si` 한 글자로 줄고, `So`·`Ss`·`Sd`·`Sa`·`Sb` 도 같다. libstdc++ 표본에서
  못 찾은 11.4% 가 거의 전부 이것이다. 표 다섯 줄을 `mangle.js` 에 넣으면 된다
- **연산자는 조회하지 않는다.** `operator delete(void*)` 가 `_ZdlPv` 라 이름이 아예 안 들어간다.
  "없음"이 아니라 "조회할 수 없음"으로 답한다
