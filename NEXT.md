...
- **여기까지 됨** — 기능 두 개.
  ...
  - **MinGW `ld` 의 `undefined reference to`** (미배포, 2026-09-20). 같은 명령이 `LNK2019` 와
    `ld` 양쪽을 읽음. 실물로 확인한 것은 아래 `ld 실물` 절에 적어 둠
...
- **다음 할 것** — `메모리 릭` 절의 30분 스파이크. 그 결과로 릭 기능을 만들지 접을지가 갈린다.
  그것과 별개로 제일 싼 후보는 이제 **MinGW 설치본의 `lib` 폴더 찾기**다 — `ld` 파싱은 붙었는데
  뒤지는 곳이 MSVC·SDK 와 워크스페이스뿐이라, MinGW 쪽 `.a` 는 워크스페이스 밖에 있으면 못 본다.
...
- **MinGW 설치본의 `lib` 폴더를 안 뒤진다.** `libDirs()` 가 아는 것은 MSVC·SDK 레이아웃뿐이다.
  워크스페이스 안의 `.a` 는 `findFiles` 가 잡으므로 프로젝트가 들고 있는 것은 걸린다
